...
function solution(m, musicinfos) {
  ...
  musicinfos.forEach((music) => {
    ...
    const duration = (new Date(`2000-01-01 ${e}:00`) - new Date(`2000-01-01 ${s}:00`)) / 1000 / 60;
    const parsedMelody = melody.replaceAll(/[A-Z]#/g, (char) => char[0].toLowerCase());
    const parsedTargetMelody = m.replaceAll(/[A-Z]#/g, (char) => char[0].toLowerCase());
    const playedMelody =
